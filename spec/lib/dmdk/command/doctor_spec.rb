...
describe DMDK::Command::Doctor do
  let(:mock_stdout) { double(:stdout, puts: nil) }
  let(:mock_stderr) { double(:stderr, puts: nil) }
  let(:successful_diagnostic) { double(DMDK::Diagnostic, success?: true, diagnose: nil, message: nil) }
  let(:failing_diagnostic) { double(DMDK::Diagnostic, success?: false, diagnose: 'error', message: 'check failed') }
  let(:diagnostics) { [] }
  ...
  before do
    allow(Shellout).to receive(:new).with('dmdk start postgresql').and_return(shellout)
  ...
  context 'with passing diagnostics' do
    ...
    it 'prints DMDK is ready.' do
      expect(mock_stdout).to receive(:puts).with('DMDK is healthy.')
      subject.run
    end
  end
