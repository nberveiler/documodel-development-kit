...
describe DMDK::Diagnostic::StaleServices do
    ...
    context 'and ps succeeds' do
      ...
      context 'but there are stale processes' do
        ...
        it 'returns help message' do
          expect(subject.detail).to eq("The following DMDK services appear to be stale:\n\nrails-web\nrails-actioncable\n\nYou can try killing them by running:\n\nkill 95010 95011\n")
        end
      end
    end
  end
  ...
  def stub_ps(result, success: true)
    ...
    full_command = %(pgrep -l -P 1 -f "runsv (grafana|jaeger|mattermost|minio|nginx|openldap|postgresql|prometheus|registry|rails-actioncable|rails-background-jobs|rails-web|redis|sshd|tunnel_|webpack)")
