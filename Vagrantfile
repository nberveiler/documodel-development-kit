...
def enable_shares(config, nfs)
  ...
  config.vm.synced_folder ".", "/vagrant",  type: "rsync",
                                            rsync__exclude: %w[documodel postgresql],
                                            rsync__auto: false
  config.vm.synced_folder "documodel/", "/vagrant/documodel", create: true, nfs: nfs
end
...
$user_setup = <<COMMANDS # rubocop:disable Style/GlobalVars
  ...
  sudo ln -s /vagrant /home/$DEV_USER/documodel-development-kit

  # automatically move into the documodel-development-kit folder, but only add the command
  # if it's not already there
  if [ -f /home/$DEV_USER/.bash_profile ]; then
    sudo -u $DEV_USER -i bash -c "grep -q \"cd /home/$DEV_USER/documodel-development-kit/\" /home/$DEV_USER/.bash_profile || echo \"cd /home/$DEV_USER/documodel-development-kit/\" >> /home/$DEV_USER/.bash_profile"
  else
    sudo -u $DEV_USER -i bash -c "touch /home/$DEV_USER/.bash_profile && echo \"cd /home/$DEV_USER/documodel-development-kit/\" >> /home/$DEV_USER/.bash_profile"
  fi

  # set up dmdk
  echo '/vagrant' > /vagrant/.dmdk-install-root
  sudo -u $DEV_USER -i bash -c "gem install documodel-development-kit"
  sudo -u $DEV_USER -i bash -c "dmdk trust /vagrant"
COMMANDS
