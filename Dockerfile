...
LABEL authors.maintainer "DMDK contributors: https://gitlab.com/gitlab-org/gitlab-development-kit/graphs/master"

# Directions when writing this dockerfile:
# Keep least changed directives first. This improves layers caching when rebuilding.

RUN useradd --user-group --create-home dmdk
...
FROM base AS rbenv
WORKDIR /home/dmdk
RUN echo 'export PATH="/home/dmdk/.rbenv/bin:$PATH"' >> .bash_profile
RUN echo 'eval "$(rbenv init -)"' >> .bash_profile
COPY --from=source-rbenv --chown=dmdk /rbenv .rbenv
COPY --from=source-ruby-build --chown=dmdk /ruby-build .rbenv/plugins/ruby-build
USER dmdk
...
COPY --from=rbenv --chown=dmdk /home/dmdk/ .
