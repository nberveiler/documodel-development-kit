require 'erb'
require 'fileutils'
require_relative '../dmdk/config'

module Runit
  class Config
    attr_reader :dmdk_root
    ...
    def initialize(dmdk_root)
      @dmdk_root = dmdk_root
    end

    def log_dir
      File.join(dmdk_root, 'log')
    end

    def services_dir
      File.join(dmdk_root, 'services')
    end

    def sv_dir
      File.join(dmdk_root, 'sv')
    end

    def run_env
      @run_env ||= DMDK::Config.new.dump_run_env!
    end
    ...
    def create_runit_log_service(service, max_service_length, index)
      ...
      log_prefix = DMDK::Output.ansi(DMDK::Output.color(index))
      log_label = format("%-#{max_service_length}s : ", service.name)
      reset_color = DMDK::Output.reset_color
