...
module Runit
  SERVICE_SHORTCUTS = {
    ...
    'tunnel' => 'tunnel_*',
    'db' => '{redis,postgresql}',
    'rails-migration-dependencies' => '{redis,postgresql}'
  }.freeze
  ...
  def self.start_runsvdir
    ...
    runit_config = Runit::Config.new(DMDK.root)
    runit_config.render

    # It is important that we use an absolute path with `runsvdir`: this
    # allows us to distinguish processes belonging to different DMDK
  ...
  def self.runsvdir_base_args
    ['runsvdir', '-P', DMDK.root.join('services').to_s]
  end
  ...
  def self.runit_installed!
    ...
    abort <<~MESSAGE
      ...
      ERROR: documodel-development-kit requires Runit to be installed.
  ...
  def self.stop
    DMDK::Output.notice "Shutting all services: "
    ...
    2.times do
      ...
      DMDK::Output.notice 'retrying stop'
    end

    unless stopped
      DMDK::Output.error 'stop failed'
    ...
    DMDK::Output.notice "Shutting down runsvdir (pid #{pid})"
    Process.kill('HUP', pid)

    DMDK::Output.puts
    DMDK::Output.success "All services are shutdown!"
  end
  ...
  def self.service_shortcut(svc)
    ...
    if glob.include?('/')
      DMDK::Output.error "invalid service shortcut: #{svc} -> #{glob}"
  ...
  def self.wait_runsv!(dir)
    unless File.directory?(dir)
      DMDK::Output.error "unknown runit service: #{dir}"
    ...
    DMDK::Output.error "timeout waiting for runsv in #{dir}"
