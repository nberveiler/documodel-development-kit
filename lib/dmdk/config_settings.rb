...
module DMDK
  class ConfigSettings
    ...
    def read_or_write!(filename, value)
      ...
      File.write(DMDK.root.join(filename), value)
      value
    end
    ...
    def config_file_protected?(target)
      return false if dmdk.overwrite_changes

      dmdk.protected_config_files&.any? { |pattern| File.fnmatch(pattern, target) }
    end
    ...
    def redis_socket
      dmdk_root.join('redis/redis.socket')
    end

    def inspect
      "#<DMDK::ConfigSettings slug:#{slug}>"
    end
    ...
    def sanitized_read!(filename)
      File.read(DMDK.root.join(filename)).chomp
    end
  end
end
