...
module DMDK
  module Command
    class Doctor
      def initialize(diagnostics: DMDK::Diagnostic.all, stdout: $stdout, stderr: $stderr)
      ...
      def start_necessary_services
        Shellout.new('dmdk start postgresql').run
      end

      def show_healthy
        stdout.puts 'DMDK is healthy.'
      end
      ...
      def warning
        <<~WARNING
          ...
          help you when you encounter issues with DMDK.
          If your DMDK is working fine, you can safely ignore them. Thanks!
          #{'=' * 80}
